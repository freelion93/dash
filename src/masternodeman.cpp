...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if(fAcceptable){
            ...
            // should be at least not earlier than block when 1000 MTUCICOIN tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            ...
            {
                CBlockIndex* pMNIndex = (*mi).second; // block for 1000 MTUCICOIN tx -> 1 confirmation
