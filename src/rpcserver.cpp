...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Mtucicoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Mtucicoin server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    /* Mtucicoin features */
    { "mtucicoin",               "masternode",             &masternode,             true,      true,       false },
    { "mtucicoin",               "masternodelist",         &masternodelist,         true,      true,       false },
    { "mtucicoin",               "mnbudget",               &mnbudget,               true,      true,       false },
    { "mtucicoin",               "mnbudgetvoteraw",        &mnbudgetvoteraw,        true,      true,       false },
    { "mtucicoin",               "mnfinalbudget",          &mnfinalbudget,          true,      true,       false },
    { "mtucicoin",               "mnsync",                 &mnsync,                 true,      true,       false },
    { "mtucicoin",               "spork",                  &spork,                  true,      true,       false },
#ifdef ENABLE_WALLET
    { "mtucicoin",               "darksend",               &darksend,               false,     false,      true  }, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use mtucicoind, or the -server option to mtucicoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> mtucicoin-cli " + methodname + " " + args + "\n";
}
